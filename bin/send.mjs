...
import {
  ...
  synthCalendar, confirmToken, offHours,
} from '../dm.mjs';
...
// ⚠⚠ 2026-09-11 事故：`--at 苏澜`（通讯录里本名是「苏澜 Lena」）没匹配上，旧代码
//   `return (p && p.md_account_id) || a` 把没查到的名字**原样当成 accountId** 发了出去，
//   服务端存成 `[aid]苏澜[/aid]`，UI 上就是一串裸标记，人没被 @ 到、也没有任何报错。
//   （SKILL.md 里「查不到人直接拒发」那句当时是假的，就是它让人放心用了错名字。）
//   现在改成真拒发：一个名字查不到就整条不发，并把相近的候选打出来。
export function resolveAtIds(atList, people, flagName = '--at') {
  const ids = [];
  const misses = [];
  for (const a of atList || []) {
    const p = (people || []).find((c) => c.name === a || c.nickname === a
      || c.md_account_id === a || c.en_name === a
      || (Array.isArray(c.aliases) && c.aliases.includes(a)));
    if (p && p.md_account_id) ids.push(p.md_account_id);
    else misses.push(a);
  }
  if (misses.length) {
    const lines = misses.map((m) => {
      const near = (people || [])
        .filter((c) => c.name && (c.name.includes(m) || m.includes(c.name)
          || (c.nickname && (c.nickname.includes(m) || m.includes(c.nickname)))))
        .slice(0, 5).map((c) => `      ${flagName} "${c.name}"`);
      return `    「${m}」查无此人`
        + (near.length ? `，是不是这几个之一：\n${near.join('\n')}` : '（通讯录里连相近的都没有）');
    });
    return { error: `拒绝发送：${flagName} 的名字在 contactmd/contacts.json 里没查到，不猜也不原样当 accountId 发（发出去就是一串没人被 @ 到的 [aid] 裸标记）。\n${lines.join('\n')}` };
  }
  return { ids };
}
...
function parseArgs(argv) {
  const out = {
    ...
    skipRecheck: false, why: '', task: '', post: '', group: '', calendar: '', at: [],
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    // 主动在某个日程下留一条评论（不是回复该日程已收到的通知）。
    else if (a === '--calendar') out.calendar = String(argv[++i] || '');
...
export function tierOf({
  ...
  if (auto) return '🟢';
  if (wantDm || isGroup || isTask || isRecord || isPost || isCalendar) return '🔴';
  return '🟡';
}
...
async function main() {
  ...
  const wantCalendar = !!args.calendar;
  const wantDm = !!args.to
    || (!wantRecord && !wantTask && !wantPost && !wantGroup && !wantFeed && !wantCalendar && !!args.accountId);

  if (wantFeed && (args.seg || args.to || wantRecord || wantTask || wantPost || wantGroup || wantCalendar)) {
    console.log('拒绝发送：--feed（动态+群卡片）和 --seg / --to / --record / --task / --post / --group / --calendar 只能给一个。');
    return 1;
  }

  if (wantRecord && (args.seg || args.to || args.task || wantPost || wantGroup || wantCalendar)) {
    console.log('拒绝发送：--record（主动在记录讨论下留言）和 --seg / --to / --task / --post / --group / --calendar 只能给一个。');
    return 1;
  }
  if (wantTask && (args.seg || args.to || wantRecord || wantPost || wantGroup || wantCalendar)) {
    console.log('拒绝发送：--task（主动在任务下留言）和 --seg / --to / --record / --post / --group / --calendar 只能给一个。');
    return 1;
  }
  if (wantPost && (args.seg || args.to || wantRecord || wantTask || wantGroup || wantCalendar)) {
    console.log('拒绝发送：--post（主动在动态下留言）和 --seg / --to / --record / --task / --group / --calendar 只能给一个。');
    return 1;
  }
  if (wantGroup && (args.seg || args.to || wantRecord || wantTask || wantPost || wantCalendar)) {
    console.log('拒绝发送：--group（主动往群里发消息）和 --seg / --to / --record / --task / --post / --calendar 只能给一个。');
    return 1;
  }
  if (wantCalendar && (args.seg || args.to || wantRecord || wantTask || wantPost || wantGroup)) {
    console.log('拒绝发送：--calendar（主动在日程下留言）和 --seg / --to / --record / --task / --post / --group 只能给一个。');
    return 1;
  }
  if (wantDm && (args.seg || wantTask || wantRecord || wantPost || wantGroup || wantCalendar)) {
    console.log('拒绝发送：--to（主动发起私信）和 --seg / --task / --record / --post / --group / --calendar 只能给一个。');
    return 1;
  }
  if ((!args.seg && !wantDm && !wantTask && !wantRecord && !wantPost && !wantGroup && !wantFeed && !wantCalendar) || !args.text.trim()) {
  ...
  console.log('　　　　（长内容的正规形态：动态承载正文 + 群聊卡片引流，同样走 🔴 两步确认码；不加 --share-org 就只发那一个群）');
    console.log('　　　node bin/send.mjs --calendar <日程id> --text "正文" [--at <姓名>]... [--reply-id <id>] [--filed ...]');
    console.log('　　　　（主动在一条日程下留言，不是回复该日程已收到的通知，同样走 🔴 两步确认码；不支持 --file）');
    return 1;
  }
  ...
  const willAutoMention = (wantPost || wantTask || wantRecord) && !!args.accountId
    || (wantCalendar && args.at.length > 0);
  if (willAutoMention && LITERAL_AT_LEAD.test(args.text)) {
    console.log('拒绝发送：正文开头写了字面 @，但这条路的 @ 是根据 --account-id / --at 自动生成的');
    console.log('   （动态评论/任务评论/记录讨论/日程评论都用 [aid] 标记，服务端才会推通知、UI 才会高亮）。');
    console.log('   两个 @ 会同时出现。把正文开头那个 @xxx 删掉重发，@ 交给 --account-id / --at 生成。');
    return 1;
  }
  ...
  if (args.auto && (wantDm || wantTask || wantRecord || wantPost || wantGroup || wantFeed || wantCalendar)) {
    console.log('拒绝发送：--auto 只用于回复（--seg）。主动发起私信 / 任务留言 / 记录讨论 / 动态评论 / 群消息 / 日程评论一律走 🔴 两步确认码。');
    return 1;
  }
  ...
  if (wantDm) {
    ...
    // 称呼门按点名的那个人判：给了 --account-id 就按那个人，没给就退回全表判定。
    const owner = args.accountId
      ? (gate.people.find((c) => c.md_account_id === args.accountId) || {})
      : {};
    // `--at <姓名>`（可重复）反查 contactmd/contacts.json 拿 accountId——跟 --post
    // 那条一模一样的解法：查不到本地名单就退回 accountId 原样传（多半本来就是 id），
    // 从不在这一层猜测或拼错就算了，真正兜底的校验在 hap-cli calendar comment 的
    // --at（那边走的是 HAP 自己的实时通讯录，两层独立校验）。
    const atResolved = resolveAtIds(args.at, gate.people);
    if (atResolved.error) {
      console.log(atResolved.error);
      return 1;
    }
    const mentionAccountIds = atResolved.ids;
    item = synthCalendar({
      calendarId: args.calendar,
      name: owner.name || '',
      accountId: args.accountId,
      eventName: args.recordName,
    ...
    const atResolvedPost = resolveAtIds(args.at, gate.people);
    if (atResolvedPost.error) {
      console.log(atResolvedPost.error);
      return 1;
    }
    ...
    const atResolvedRec = resolveAtIds(args.at, gate.people);
    if (atResolvedRec.error) {
      console.log(atResolvedRec.error);
      return 1;
    }
    item = synthFeed({
      groupId,
      ...
      filed: parseFiled(args.filed),
      mentionAccountIds: atResolvedRec.ids,
    });
  } else {
  ...
  // ⚠ 2026-09-07 补：`hap calendar comment` 没有 `--attach`（hap-cli 里从没做过这个
  //   复合命令），日程评论的 sendVia 分支也没调 getFilePaths()——传 --file 会被
  //   silently 吞掉，跟当年 mail 那次一模一样。`kind` 是 'notice'，跟 task/record
  //   共用同一个值却支持不一样，不能只按 kind 判，要单独认 replyViaOf() === 'calendar'。
  const kind = typeOf(item);
  const NO_ATTACHMENT_SUPPORT = new Set(['mail']); // 只剩邮件这一条真做不到
  const isCalendarComment = kind === 'notice' && replyViaOf(item) === 'calendar';
  const fileChannelOk = !NO_ATTACHMENT_SUPPORT.has(kind) && !isCalendarComment;
  if (args.files.length && !fileChannelOk) {
    console.log(`\n拒绝发送：--file 指的这条通道（${item.sourceLabel || kind}）不支持带附件——`
      + '这条路的适配器从不读附件参数，传了也会被静默丢弃。'
  ...
  // 主动发起的日程评论（--calendar）同样归 🔴：受众是这条日程的全体参与人。
  // （isCalendarComment 已经在附件能力那道闸那儿算过一次，这里不重复声明。）
  ...
  const toId = (isTaskComment && item.target && item.target.taskId)
    ...
    || (isCalendarComment && item.target && item.target.calendarId)
    || pre.to.accountId
  ...
  // ---------- 🔴 这一档：Andy 那一眼（两步确认码） ----------
  //
  // 六种走这儿：主动发起私信（`--to`）、群消息、任务评论、记录讨论、主动发起的动态评论、日程评论。
  if (wantDm || isGroup || isTaskComment || isRecordComment || isPostComment || isCalendarComment) {
    const key = wantDm
      ? dmTo.accountId
    ...
    const off = ((!wantDm && !wantTask && !wantRecord && !wantPost && !wantGroup && !wantFeed && !wantCalendar) || args.offHours) ? '' : offHours();
    if (off) {
